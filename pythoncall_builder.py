...
class WrapClass:
    ...
    def handle_event_dispatcher(self, decorator: ast.Call, func_ptrs: List[Function], new_func_pointers: List[Function]):
        ...
        if len(decorator.args) != 0:
            ...
            if f"{'json'}" not in used_types:
                used_types.append(f"{'json'}")
            #func.
...
class WrapClass2(WrapClass):

    def __init__(self):
        super(WrapClass2, self).__init__()

    @staticmethod
    def json_export(string:str) -> list:
        module = ast.parse(string.replace("List[","["))
        wrap_list = []
...
class PythonCallBuilder():
    ...
    def gen_cython_class(self,wrap:WrapClass,call_var:str,fill_struct:str):
        ...
        if self.dispatch_mode:
            ...
            events = [f"{evt}" for evt in wrap.global_events]
            #wrap.enum_list[0][1])
    ...
    def generate_pyx(self):
        ...
        for wrap in classes:
            ...
            if len(wrap.enum_list) != 0:
                # s = [f"on_{event}" for event in wrap.enum_list[0][1]]
                # export_enum_list = f"cdef list {wrapevent.calltitle}_events = {events}"
                events = [f"{event}" for event in wrap.enum_list[0][1]]
